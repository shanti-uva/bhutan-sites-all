                    ...
                    if (path) {
                        ...
                        tree.loadKeyPath(path, function (key, status) {
                            ...
                            function () {
                                ...
                                tree.activateKey(String(focus_id));
                            }
                        )
                    } else {
                        if (plugin.settings.expand_path) {
                            /* if (DEBUG) */ console.log("expand_path = " + plugin.settings.expand_path);
        ...
        showPaths: function (paths, callback) {
            ...
            if (paths !== null) {
                if (pathlist.length == 0) { // all paths to show have already been loaded
                    ...
                    if (DEBUG)
                        log("filterNodes returned: " + ret);
                    if (callback) callback();
                }
                else { // need to load paths
                    this.loadKeyPath(this.element.fancytree("getTree"), pathlist,
                        function (node, state) {
