    ...
    attach: function (context, settings) {
      ...
      $('#kmaps-search', context).once('kmaps-explorer').each(function () {
        ...
        $tree.kmapsTree({
         ...
         // When root_kmapid is multiple nodes, the node ids are separated by spaces, if this is the case, use the first item in the data path which begins with a slash.
          var aid =  (root_kmapid.split(' ').length > 1) ? data.path.split('/')[1] : root_kmapid;
          Drupal.ajax["ajax-id-" + aid].createAction(data.key, domain);
